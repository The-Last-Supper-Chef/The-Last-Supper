...
import com.goorm.thelastsupper.reservation.service.OpenSlotsService;
...
public class SlotOpeningController {
	...
	private final OpenSlotsService openSlotsService;
	...
	public ResponseEntity<ApiResponse<OpenSlotsResponse>> openSlots(
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.successWithMessage(
			openSlotsService.execute(commandPayload), "슬롯이 성공적으로 열렸습니다."
		));
	}
}
